...
class FileHelper extends \yii\helpers\FileHelper
{
    ...
    public static function read($file)
    {
        ...
        if (($contents = fread($handle, filesize($file))) == false) {
            return false;
        }
        fclose($handle);
    ...
    {
        if (($handle = fopen($file, $flag)) == false) {
            return false;
        }
    ...
    public static function searchR(
        $folder,
        $pattern = null,
        $depth = -1,
    ...
    {
        if (is_file($folder_or_file)) {
            unlink($folder_or_file);
        } else {
            self::deleteR($folder_or_file, $pattern, 0);
        }
    }
}